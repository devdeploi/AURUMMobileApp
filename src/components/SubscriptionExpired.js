...
const SubscriptionExpired = ({ user, onRenew, existingPlanCount, plans, onRefreshPlans }) => {
    ...
    const handleRenew = async () => {
        ...
        try {
            ...
            // 1. Create Order
            const { data } = await axios.post(`${APIURL}/merchants/create-renewal-order`, {
                plan: selectedPlan,
                billingCycle
            }, config);
            ...
            RazorpayCheckout.open(options).then(async (rzpData) => {
                ...
                try {
                    ...
                    if (verifyRes.data.success) {
                        setShowSuccess(true);
                        setTimeout(() => onRenew(verifyRes.data.merchant), 3000);
                    } else {
                ...
                // handle failure
                setAlertConfig({ visible: true, title: "Error", message: `Payment Failed: ${error.description || 'Cancelled'}`, type: 'error' });
                setLoading(false);
            });

        } catch (err) {
            console.error(err);
            setAlertConfig({ visible: true, title: "Error", message: err.response?.data?.message || 'Renewal initiation failed.', type: 'error' });
            setLoading(false);
        }
    };
            ...
            <ScrollView contentContainerStyle={styles.container}>
                ...
                >
                    {loading ? (
                        ...
                        <Text style={styles.renewButtonText}>Proceed to Payment</Text>
                    )}
                </TouchableOpacity>
