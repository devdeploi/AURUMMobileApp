...
const AdDisplay = ({ ads, visible: initialVisible = true, paused = false }) => {
    ...
    useEffect(() => {
        if (ads && ads.length > 0) {
            // Seed lastShownMap with current time for all ads if they're new to the map
            ...
            const frequencyTimer = setInterval(checkAndDisplayAd, 60000);
    ...
    // Use dedicated components for Brand Ads (Fallbacks)
